...
# mnist = input_data.read_data_sets('F:/MyDownloads/mnist_data/')
...
img_path = 'F:/MyDownloads/avatar_data/user1'
img_files = data_util.get_path_files(img_path)
...
def get_generator(noise_img, output_dim, is_train=True, alpha=0.01):
    ...
    with tf.variable_scope("generator", reuse=(not is_train)):
        ...
        #
        # # 4x4x512 to 16x16x256
        # layer2 = tf.layers.conv2d_transpose(layer1, filters=256, kernel_size=3, strides=4, padding='same')
        # layer2 = tf.layers.batch_normalization(layer2, training=is_train)
        # layer2 = tf.maximum(alpha * layer2, layer2)
        # layer2 = tf.nn.dropout(layer2, keep_prob=0.8)
        #
        ...
        #
        # # 16x16x256 to 64x64x128
        # layer3 = tf.layers.conv2d_transpose(layer2, 128, 3, strides=4, padding='same')
        # layer3 = tf.layers.batch_normalization(layer3, training=is_train)
        # layer3 = tf.maximum(alpha * layer3, layer3)
        # layer3 = tf.nn.dropout(layer3, keep_prob=0.8)
        #
        ...
        #
        # # 64x64x128 to 256x256x1
        # logits = tf.layers.conv2d_transpose(layer3, output_dim, 3, strides=4, padding='same')
        #
...
def get_discriminator(inputs_img, reuse=False, alpha=0.01):
    ...
    with tf.variable_scope("discriminator", reuse=reuse):
        ...
        # # layer1 = tf.nn.dropout(layer1, keep_prob=0.8)
        #
        # # 256x256x1 to 64x64x128
        # layer1 = tf.layers.conv2d(inputs_img, 128, 3, strides=4, padding='same')
        # layer1 = tf.maximum(alpha * layer1, layer1)
        # layer1 = tf.nn.dropout(layer1, keep_prob=0.8)
        #
        ...
        # # layer2 = tf.nn.dropout(layer2, keep_prob=0.8)
        #
        # # 64x64x128 to 16x16x256
        # layer2 = tf.layers.conv2d(layer1, 256, 3, strides=4, padding='same')
        # layer2 = tf.layers.batch_normalization(layer2, training=True)
        # layer2 = tf.maximum(alpha * layer2, layer2)
        # layer2 = tf.nn.dropout(layer2, keep_prob=0.8)
        #
        ...
        # # layer3 = tf.nn.dropout(layer3, keep_prob=0.8)
        #
        # # 16x16x256 to 4x4x512
        # layer3 = tf.layers.conv2d(layer2, 512, 3, strides=4, padding='same')
        # layer3 = tf.layers.batch_normalization(layer3, training=True)
        # layer3 = tf.maximum(alpha * layer3, layer3)
        # layer3 = tf.nn.dropout(layer3, keep_prob=0.8)
        #
        ...
        # 第一层不加入BN
        layer1 = tf.layers.conv2d(inputs_img, 32, 3, strides=2, padding='same')
        layer1 = tf.maximum(alpha * layer1, layer1)
        layer1 = tf.nn.dropout(layer1, keep_prob=0.8)

        layer10 = tf.layers.conv2d(layer1, 64, 3, strides=2, padding='same')
        layer10 = tf.layers.batch_normalization(layer10, training=True)
        layer10 = tf.maximum(alpha * layer10, layer10)
        layer10 = tf.nn.dropout(layer10, keep_prob=0.8)

        layer11 = tf.layers.conv2d(layer10, 128, 3, strides=2, padding='same')
        layer11 = tf.layers.batch_normalization(layer11, training=True)
        layer11 = tf.maximum(alpha * layer11, layer11)
        layer11 = tf.nn.dropout(layer11, keep_prob=0.8)

        # 16 x 16 x 128 to 8 x 8 x 256
        layer2 = tf.layers.conv2d(layer11, 256, 3, strides=2, padding='same')
        layer2 = tf.layers.batch_normalization(layer2, training=True)
        layer2 = tf.maximum(alpha * layer2, layer2)
        layer2 = tf.nn.dropout(layer2, keep_prob=0.8)
...
def plot_images(samples):
    ...
    for img, ax in zip(samples, axes):
        ax.imshow(img.reshape((IMAGE_W, IMAGE_H, 3)), cmap='Greys_r')
...
def train(noise_size, data_shape, batch_size, n_samples):
    ...
    tf_img_batch = data_util.get_batch(image=img_files, image_W=IMAGE_W, image_H=IMAGE_H, batch_size=batch_size,
        ...
        try:
            ...
            for step in np.arange(max_steps):
                ...
                if step % 5000 == 0 or (step + 1) == max_steps:
                    ...
                    plot_images(samples)
                    print("Epoch {}/{}....".format(step + 1, max_steps),
...
with tf.Graph().as_default():
    train(noise_size, [-1, IMAGE_W, IMAGE_H, 3], batch_size, n_samples)
